...
(`pnpm test`, `pnpm typecheck`, `pnpm lint`, and any other relevant
checks pass), take the work through to a PR that is green and ready
for human review. Use the `gh` CLI for all GitHub interactions.
**Never merge the PR yourself — merging is a developer decision.**

**Flow:**

1. **Stage and commit** only the intended files. Inspect `git status`
...
3. **Open a PR** with `gh`:
   ...
   gh pr create \
     --base <base-branch> \
     --title "<short summary>" \
     --body "<issue reference + what changed + how it was verified>"
   ```
   Reference the issue (`Closes #N` or `Fixes #N`) in the body so
   merging the PR closes the issue.
4. **Watch status checks**: `gh pr checks --watch` (or poll with
...
5. **Stop and hand off** when checks are green. Do not run
